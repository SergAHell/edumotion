// Type: ANT_Managed_Library.IANT_Channel
...
namespace ANT_Managed_Library
{
  ...
  public interface IANT_Channel : IDisposable
  {
    ...
    /// 
    /// <returns>
    /// The channel number that is sent with messages on the ANT messaging layer
    /// </returns>
    byte getChannelNum();
    ...
    /// </returns>
    ANT_ReferenceLibrary.MessagingReturnCode sendAdvancedBurstTransfer(byte[] data, byte numStdPcktsPerSerialMsg, uint completeWaitTime);
    ...
    /// True on success. Note: Always returns true with a response time of 0
    /// </returns>
    bool SetAGCConfig(byte AGCConfigByte, uint responseWaitTime);
  }
}
